/*
##########################################################################################
Title: Classification of Weierstrass elliptic curve over Z_3
...
This is source code used for classification of elliptic curve over Z_3.
It includes calculation of No. of non-singular curves, No. of classes of EC over Z_3.
########################################################################################## 
*/
#include <iostream>
#include <bits/stdc++.h>
...
string print_curve_j_not0(ll a2, ll a6)
{
    ...
    str += "y<sup>2</sup> = x<sup>3</sup> " + a2str + a6str;
    return str;
}
string print_curve_j0(ll a4, ll a6)
{
    string str = "";
    string  a4str = "", a6str = "";
    ...
    str += "y<sup>2</sup> = x<sup>3</sup> " + a4str + a6str;
    return str;
}
ll gcd(ll a, ll b)
{
...
long long int discriminant_j_not0(ll a2,ll a6, ll p)
{
    ll delta;
    delta = ((2*modpow(a2,3,p)%p*a6%p)%p) % p;
    return delta % p;
}
long long int discriminant_j0(ll a4, ll p)
{
    ll delta;
    delta = (modpow(a4,3,p)*2)%p;
    return delta % p;
}
map<pair<string, vector<int>>, int> all_curves_1(ll p) // j!=0 char 3
{
    ...
    for (ll a2 = 0; a2 < p; a2++)
    {
        for (ll a6 = 0; a6 < p; a6++)
        {
            if (gcd(discriminant_j_not0(a2,a6, p) % p, p) == 1)
            {
                vector<int> v;
                v.push_back(a2);
                v.push_back(a6);
...
map<pair<string, vector<int>>, int> all_curves_2(ll p) // j==0 char 3
{
    map<pair<string, vector<int>>, int> mp;
        ...
        for (ll a4 = 0; a4 < p; a4++)
        {
            for (ll a6 = 0; a6 < p; a6++)
            {
                if (gcd(discriminant_j0(a4, p) % p, p) == 1)
                {
                    vector<int> v;  
                    v.push_back(a4);
                    v.push_back(a6);
                    mp[{print_curve_j0(a4, a6), v}] = 0;
                }
            }
        }
    return mp;
...
pair<string, vector<int>> transformed_curve_j_not0(int u, int a2, int a6, int p)
{
    int u_ = modinv(u, p);
    ll a2_ = (a2%p*modpow(u_,2,p)%p) % p;
    ll a6_ = (a6%p*modpow(u_,6,p)%p) % p;

    vector<int> vec;
    vec.push_back(a2_);
    vec.push_back(a6_);

    if (gcd(discriminant_j_not0(a2_,a6_, p) % p, p) == 1)
    {
...
pair<string, vector<int>> transformed_curve_j0(int u, int r,int a4, int a6, int p)
{
     int u_ = modinv(u, p);
     ll a4_ = (a4%p*modpow(u_,4,p)%p) % p;
    ll a6_ = ((modpow(r,3,p)  + (a4%p*r%p)  + a6%p) % p * (modpow(u_, 6, p)) % p)%p; // put more mods :))

    vector<int> vec;
    vec.push_back(a4_);
    vec.push_back(a6_);

    if (gcd(discriminant_j0(a4_, p) % p, p) == 1)
    {
        string str = print_curve_j0(a4_, a6_);
        return {str, vec};
    }
    else
        return {"singular curve", {}};
...
int main()
{
    ofstream file1;
    {
        file1.open("ViewClassificationOfCurvesChar3.html");
        file1 << "<!DOCTYPE html>\n";
        file1 << "<html>\n";
        file1 << "<head>\n";
        file1 << "<title>Classification Of Elliptic Curves Over Z<sub>3</sub></title>\n";
        file1 << "</head>\n";
        file1 << "<body>\n";
        file1 << "</body>\n";
        file1 << "<center>\n";
        file1 << "<h1>Classification Of Elliptic Curves Over Z<sub>3</sub>, j(E) != 0</h1>\n";
        ...
        for (auto curve = mp.begin(); curve != mp.end(); curve++)
        {
            ...
            if (color == 0)
            {
                ...
                for (ll u = 0; u < p; u++)
                {
                    if(gcd(u,p)!=1)
                        continue;   
                    pair<string, vector<int>> tcurve = transformed_curve_j_not0(u, coeff[0], coeff[1], p);
            ...
            if (color == 0)
            {
                ...
                for (ll u = 0; u < p; u++)
                {
                    ...
                    for (ll r = 0; r < p;r++)
                    {
                            ...
                            pair<string, vector<int>> tcurve = transformed_curve_j0(u, r, coeff[0], coeff[1], p);
                            if(tcurve.second == coeff)autcnt++;
                            if (tcurve.first == "singular curve")
                                continue;
                            else
